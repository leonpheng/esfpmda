...
if(grepl("\\.csv$", lst$filename[i])){
  write.csv(read.csv(sour),paste0(odir1,lst$rename[i],".csv"),row.names = F)}
    #file.copy(from=sour,
    #to=paste0(odir1,lst$rename[i],".csv"),overwrite=T)}
...
if(!grepl("\\.sas7bdat$", lst$filename[i])&!grepl("\\.csv$", lst$filename[i])&
   !grepl("\\.xpt$", lst$filename[i])&!grepl("\\.html$", lst$filename[i])){
       ...
       if(grepl("\\.csv$", lst$filename[i])){
         write.csv(read.csv(sour),paste0(odir2,lst$rename[i],".txt"),row.names = F)}
